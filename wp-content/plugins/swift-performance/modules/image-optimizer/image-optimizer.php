		...
		switch ($action){
			...
			case 'quickview':
				...
				if (file_exists(ABSPATH . $image['file'])){
					...
					$original_src	= (file_exists(ABSPATH . $image['file'] . '.swift-original') ? Swift_Performance::home_url() . $image['file'] . '.swift-original' : '');
	...
	public function process_queue(){
		...
		foreach ((array)$images as $image){
			...
			if (filesize(ABSPATH . $image['file']) > 10*1024*1024){
				Swift_Performance::log(ABSPATH . $image['file'] . ' not optimized. File exceeded maximum size.', 6);
				$wpdb->query($wpdb->prepare("UPDATE " . SWIFT_PERFORMANCE_IMAGE_TABLE . " SET status = 0 WHERE hash = %s LIMIT 1", $hash));
				continue;
			}
		...
		if ($response['status'] == 'success'){
			...
			if ($check !== false && isset($check[0]) && $check[0] > 0){
				if (filesize($file) > filesize($test_img)){
					...
					$image_url = str_replace(ABSPATH, Swift_Performance::home_url(), $file);
