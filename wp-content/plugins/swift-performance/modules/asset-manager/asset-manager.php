	...
	public $image_sizes;
      ...
      public function __construct(){
		...
		if (!defined('SWIFT_PERFORMANCE_LAZYLOAD_SIZE')){
            ...
            if (Swift_Performance::check_option('merge-scripts', 1) || Swift_Performance::check_option('merge-styles', 1) || Swift_Performance::check_option('lazy-load-images', 1) || Swift_Performance::check_option('lazyload-iframes', 1)){
                  // Prepare JS buffer
		...
		add_action('init', function( $scripts ){
			if (!Swift_Performance::is_admin() && Swift_Performance::check_option('replace-jquery', 'none', '!=')){
				wp_deregister_script('jquery');
			      wp_register_script('jquery', SWIFT_PERFORMANCE_URI . 'js/' . Swift_Performance::get_option('replace-jQuery'), false);
	...
	public function asset_manager_callback($buffer){
		...
		foreach ($html->find('link[rel="stylesheet"], style, script, img, iframe') as $node){
				...
				// Process Script
                  ...
                  if($node->tag == 'iframe'){
                        if (Swift_Performance::check_option('lazyload-iframes', 1)){
                              // Exclude iframes
		...
		$critical_css = preg_replace('~url\(([^\)]+)?(\s+)([^\)]+)?\)~','url(\'$1$2$3\')',$critical_css);
            ...
            $css_dir = apply_filters('swift_performance_css_dir', Swift_Performance::check_option('separate-css', 1) ? trailingslashit(trim(parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH),'/')) . 'css' : 'css');
            $js_dir = apply_filters('swift_performance_js_dir', Swift_Performance::check_option('separate-js', 1) ? trailingslashit(trim(parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH),'/')) . 'js' : 'js');
