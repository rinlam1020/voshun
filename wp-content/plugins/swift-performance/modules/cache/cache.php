...
class Swift_Performance_Cache {
      ...
      public function __construct(){
		...
		add_action('customize_save_after', array('Swift_Performance_Cache', 'clear_all_cache'));
            ...
            // Discover mode
            add_action('init', array($this, 'discover'));
      ...
      public function set_cache(){
            ...
            if (Swift_Performance::check_option('caching-mode', array('disk_cache_rewrite', 'disk_cache_php'), 'IN')){
                  ...
                  // General cache
                        ...
                        $timestamp = time();
                        Swift_Performance::mysql_query($wpdb->prepare("UPDATE " . SWIFT_PERFORMANCE_TABLE_PREFIX . "warmup SET timestamp = %s, type = %s WHERE id = %s LIMIT 1", $timestamp, $type, $id));
      ...
      public function discover(){
            ...
            if (Swift_Performance::check_option('discover-warmup', 1) && (Swift_Performance_Cache::is_cacheable() || Swift_Performance_Cache::is_cacheable_dynamic()) && !Swift_Performance::is_404()){
                  ...
                  $result = $wpdb->query("INSERT IGNORE INTO {$table_name} (id, url, priority, menu_item) VALUES ('".Swift_Performance::get_warmup_id($url)."', '".esc_sql($url)."', ".(int)$priority.", '0')");
      ...
      public static function is_cacheable(){
            ...
            $query_string     = (isset($_SERVER['HTTP_X_PREBUILD']) || Swift_Performance::check_option('ignore-query-string', 1) || empty($_GET));
            ...
            return apply_filters('swift_performance_is_cacheable', !self::is_exluded() && $archive && $author && $rest && $feed && $is_404 && !Swift_Performance::is_admin() && $post && $query_string && $logged_in && $ajax_cache && !self::is_crawler() && !Swift_Performance::is_password_protected());
      }
      ...
      public static function is_cacheable_dynamic(){
            ...
            if (Swift_Performance::check_option('dynamic-caching', 1)){
                  ...
                  $logged_in  = (!Swift_Performance::is_user_logged_in() || Swift_Performance::check_option('enable-caching-logged-in-users', 1));
                  $is_404     = (Swift_Performance::check_option('cache-404',1) || !Swift_Performance::is_404());
                  return apply_filters('swift_performance_is_cacheable_dynamic', !self::is_exluded() && $is_404 && $cacheable && $logged_in && !Swift_Performance::is_admin() && !self::is_crawler() && !Swift_Performance::is_password_protected());
            }
            return apply_filters('swift_performance_is_cacheable_dynamic', false);
      }
      ...
      */
      ...
      public static function get_cloudflare_zones(){
            ...
            $host       = parse_url(Swift_Performance::home_url(), PHP_URL_HOST);
