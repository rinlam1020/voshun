...
class Swift_Performance {
	...
	public function __construct() {
		...
		add_action('init', function(){
			...
			if ($_GET['swift-performance-action'] == 'clear-page-cache' && current_user_can('manage_options') && isset($_GET['_wpnonce']) && wp_verify_nonce($_GET['_wpnonce'], 'clear-swift-cache')){
	...
	public function init(){
		...
		if (!defined('SWIFT_PERFORMANCE_CACHE_URL')){
			define('SWIFT_PERFORMANCE_CACHE_URL', str_replace(ABSPATH, parse_url(home_url(), PHP_URL_SCHEME) . '://' . $_SERVER['HTTP_HOST'] . Swift_Performance::home_dir() . '/', SWIFT_PERFORMANCE_CACHE_DIR));
		}
	...
	public static function prebuild_cache_hit($permalink){
		...
		$response = wp_remote_get($permalink, array('headers' => apply_filters('swift_performance_prebuild_headers',array('X-merge-assets' => 'true', 'X-Prebuild' => 'true')), 'user-agent' => 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10.12; rv:52.0) Gecko/20100101 Firefox/52.0', 'timeout' => 120));
		$response_object = $response['http_response']->get_response_object();

		// Skip on error
		if (is_wp_error($response)){
			Swift_Performance::log('Prebuild cache stopped due an error: ' . $response->get_error_message(), 1);
		...
		if (Swift_Performance::check_option('mobile-support', 1)){
			$response = wp_remote_get($permalink, array('headers' => apply_filters('swift_performance_mobile_prebuild_headers', array('X-merge-assets' => 'true', 'X-Prebuild' => 'true')), 'user-agent' => 'Mozilla/5.0 (iPhone; CPU iPhone OS 6_0 like Mac OS X) AppleWebKit/536.26 (KHTML, like Gecko) Version/6.0 Mobile/10A5376e Safari/8536.25', 'timeout' => 120));
	...
	public static function options_saved(){
		...
		if(isset($data['swift_performance_options']['remote-cron']) && $data['swift_performance_options']['remote-cron'] == 1){
			$purchase_key 	= (isset($data['swift_performance_options']['purchase-key']) ? $data['swift_performance_options']['purchase-key'] : '');
			$frequency		= (isset($data['swift_performance_options']['remote-cron-frequency']) ? $data['swift_performance_options']['remote-cron-frequency'] : 'daily');
	...
	public static function build_rewrite_rules(){
		...
		try{
			...
			// Custom htaccess
			if (isset($data['swift_performance_options']['custom-htaccess']) && !empty($data['swift_performance_options']['custom-htaccess']) && $server_software == 'apache'){
	...
	public static function set_thread($is_available){
		if (!defined('SWIFT_PERFORMANCE_THREAD')){
			Swift_Performance::log('Thread for ' . (is_ssl() ? 'https://' : 'http://') . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'] . ' is ' . ($is_available ? 'available' : 'not available'), 9);
	...
	public static function lock_thread($hook){
		...
		Swift_Performance::log('Lock thread for ' . (is_ssl() ? 'https://' : 'http://') . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'], 9);
	}
	...
	public static function unlock_thread(){
		...
		Swift_Performance::log('Unlock thread for ' . (is_ssl() ? 'https://' : 'http://') . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'], 9);
	}
	...
	public static function is_rest(){
		...
		if (preg_match('~'.preg_quote($rest_url).'~', Swift_Performance::home_url() . $_SERVER['REQUEST_URI'])){
			return true;
		}
		return false;
	}
	...
	public static function update_plugin_header(){
		...
		if (defined('SWIFT_PERFORMANCE_WHITELABEL') || doing_action('swift_performance_options_saved')){

			if (isset($_REQUEST['data'])){
				global $swift_performance_options;
				parse_str(urldecode($_REQUEST['data']), $data);
				$swift_performance_options = $data['swift_performance_options'];
			}
			...
			$plugin_header[] = " * Description: " . Swift_Performance::get_option('whitelabel-plugin-description');
