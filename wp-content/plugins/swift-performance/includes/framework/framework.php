<?php
    ...
    if ( ! class_exists( 'ReduxSAFrameworkInstances' ) ) {
        // Instance Container
        require_once dirname( __FILE__ ) . '/inc/class.reduxsa_instances.php';
        require_once dirname( __FILE__ ) . '/inc/lib.reduxsa_instances.php';
    }

    if ( class_exists( 'ReduxSAFrameworkInstances' ) ) {
        add_action( 'reduxsa/init', 'ReduxSAFrameworkInstances::get_instance' );
    }
    ...
    if ( ! class_exists( 'ReduxSAFramework' ) ) {
        ...
        // ReduxSA CDN class
        require_once dirname( __FILE__ ) . '/inc/class.reduxsa_cdn.php';

        // ReduxSA API class  :)
        require_once dirname( __FILE__ ) . '/inc/class.reduxsa_api.php';

        // General helper functions
        require_once dirname( __FILE__ ) . '/inc/class.reduxsa_helpers.php';

        // General functions
        require_once dirname( __FILE__ ) . '/inc/class.reduxsa_functions.php';
        require_once dirname( __FILE__ ) . '/inc/class.p.php';

        require_once dirname( __FILE__ ) . '/inc/class.thirdparty.fixes.php';

        require_once dirname( __FILE__ ) . '/inc/class.reduxsa_filesystem.php';

        require_once dirname( __FILE__ ) . '/inc/class.reduxsa_admin_notices.php';

        // ThemeCheck checks
        require_once dirname( __FILE__ ) . '/inc/themecheck/class.reduxsa_themecheck.php';

        // Welcome
        require_once dirname( __FILE__ ) . '/inc/welcome/welcome.php';

        /**
         * Main ReduxSAFramework class
         *
         * @since       1.0.0
         */
        class ReduxSAFramework {
            ...
            public static $_as_plugin = true;

            public static function init() {
                $dir = ReduxSA_Helpers::cleanFilePath( dirname( __FILE__ ) );

                // Windows-proof constants: replace backward by forward slashes. Thanks to: @peterbouwmeester
                self::$_dir           = trailingslashit( $dir );
                self::$wp_content_url = trailingslashit( ReduxSA_Helpers::cleanFilePath( ( is_ssl() ? str_replace( 'http://', 'https://', WP_CONTENT_URL ) : WP_CONTENT_URL ) ) );

                // See if ReduxSA is a plugin or not
                if ( strpos( ReduxSA_Helpers::cleanFilePath( __FILE__ ), ReduxSA_Helpers::cleanFilePath( get_stylesheet_directory() ) ) !== false || strpos( ReduxSA_Helpers::cleanFilePath( __FILE__ ), ReduxSA_Helpers::cleanFilePath( get_template_directory_uri() ) ) !== false || strpos( ReduxSA_Helpers::cleanFilePath( __FILE__ ), ReduxSA_Helpers::cleanFilePath( WP_CONTENT_DIR . '/themes/' ) ) !== false ) {
                    ...
                    if ( strpos( self::$_dir, ABSPATH ) === false ) {
                        ...
                        foreach ( get_plugins() as $key => $value ) {
                            if ( is_plugin_active( $key ) && strpos( $key, 'reduxsa-framework.php' ) !== false ) {
                                self::$_dir = trailingslashit( ReduxSA_Helpers::cleanFilePath( WP_CONTENT_DIR . '/plugins/' . plugin_dir_path( $key ) . 'ReduxSACore/' ) );
                                $is_plugin  = true;
                            }
                        }
                        if ( ! $is_plugin ) {
                ...
                if ( self::$_is_plugin == true || self::$_as_plugin == true ) {
                    ...
                    if ( strpos( ReduxSA_Helpers::cleanFilePath( __FILE__ ), ReduxSA_Helpers::cleanFilePath( get_template_directory() ) ) !== false ) {
                        $relative_url = str_replace( ReduxSA_Helpers::cleanFilePath( get_template_directory() ), '', self::$_dir );
                        self::$_url   = trailingslashit( get_template_directory_uri() . $relative_url );
                    } else if ( strpos( ReduxSA_Helpers::cleanFilePath( __FILE__ ), ReduxSA_Helpers::cleanFilePath( get_stylesheet_directory() ) ) !== false ) {
                        $relative_url = str_replace( ReduxSA_Helpers::cleanFilePath( get_stylesheet_directory() ), '', self::$_dir );
                        self::$_url   = trailingslashit( get_stylesheet_directory_uri() . $relative_url );
                    } else {
                        $wp_content_dir = trailingslashit( ReduxSA_Helpers::cleanFilePath( WP_CONTENT_DIR ) );
                ...
                self::$_url       = apply_filters( "reduxsa/_url", self::$_url );
                self::$_dir       = apply_filters( "reduxsa/_dir", self::$_dir );
                self::$_is_plugin = apply_filters( "reduxsa/_is_plugin", self::$_is_plugin );
            }

            // ::init()

            public $framework_url = 'http://www.reduxsaframework.com/';
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                if ( isset ( $_POST ) && isset ( $_POST['action'] ) && $_POST['action'] == 'heartbeat' ) {
                    ...
                    // Hook, for purists.
                    if ( ! has_action( 'reduxsa/ajax/heartbeat' ) ) {
                        do_action( 'reduxsa/ajax/heartbeat', $this );
                    }
                ...
                ReduxSA_Functions::$_parent     = $this;
                ReduxSA_CDN::$_parent           = $this;
                ReduxSA_Admin_Notices::$_parent = $this;
                ...
                if ( empty ( $this->args['footer_credit'] ) ) {
                    $this->args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( __( 'Options panel created using %1$s', 'reduxsa-framework' ), '<a href="' . esc_url( $this->framework_url ) . '" target="_blank">' . __( 'ReduxSA Framework', 'reduxsa-framework' ) . '</a> v' . self::$_version ) . '</span>';
                }

                if ( empty ( $this->args['menu_title'] ) ) {
                    $this->args['menu_title'] = __( 'Options', 'reduxsa-framework' );
                }

                if ( empty ( $this->args['page_title'] ) ) {
                    $this->args['page_title'] = __( 'Options', 'reduxsa-framework' );
                }

                $this->old_opt_name = $this->args['opt_name'];

                /**
                 * filter 'reduxsa/args/{opt_name}'
                 *
                 * @param  array $args ReduxSAFramework configuration
                 */
                $this->args = apply_filters( "reduxsa/args/{$this->args['opt_name']}", $this->args );

                /**
                 * filter 'reduxsa/options/{opt_name}/args'
                 *
                 * @param  array $args ReduxSAFramework configuration
                 */
                $this->args = apply_filters( "reduxsa/options/{$this->args['opt_name']}/args", $this->args );
                ...
                if ( ! empty ( $this->args['opt_name'] ) ) {
                    ...
                    /**
                     * filter 'reduxsa-sections'
                     *
                     * @deprecated
                     *
                     * @param  array $sections field option sections
                     */
                    $this->sections = apply_filters( 'reduxsa-sections', $sections ); // REMOVE LATER
                    /**
                     * filter 'reduxsa-sections-{opt_name}'
                     *
                     * @deprecated
                     *
                     * @param  array $sections field option sections
                     */
                    $this->sections = apply_filters( "reduxsa-sections-{$this->args['opt_name']}", $this->sections ); // REMOVE LATER
                    /**
                     * filter 'reduxsa/options/{opt_name}/sections'
                     *
                     * @param  array $sections field option sections
                     */
                    $this->sections = apply_filters( "reduxsa/options/{$this->args['opt_name']}/sections", $this->sections );

                    /**
                     * Construct hook
                     * action 'reduxsa/construct'
                     *
                     * @param object $this ReduxSAFramework
                     */
                    do_action( 'reduxsa/construct', $this );
                    ...
                    $this->filesystem = ReduxSA_Filesystem::get_instance( $this );

                    //set reduxsa upload folder
                    $this->set_reduxsa_content();
                    ...
                    // Tracking
                    if ( isset( $this->args['allow_tracking'] ) && $this->args['allow_tracking'] && ReduxSA_Helpers::isTheme( __FILE__ ) ) {
                        $this->_tracking();
                    }
                    ...
                    if ( $this->args['database'] == "network" && $this->args['network_admin'] ) {
                        add_action( 'network_admin_edit_reduxsa_' . $this->args['opt_name'], array(
                            $this,
                            'save_network_page'
                        ), 10, 0 );
                    ...
                    if ( $this->args['dev_mode'] == true || ReduxSA_Helpers::isLocalHost() == true ) {
                        require_once 'core/dashboard.php';
                        new reduxsaDashboardWidget( $this );

                        if ( ! isset ( $GLOBALS['reduxsa_notice_check'] ) ) {
                            require_once 'core/newsflash.php';

                            $params = array(
                                'dir_name'    => 'notice',
                                'server_file' => 'http://reduxsaframework.com/wp-content/uploads/reduxsa/reduxsa_notice.json',
                                'interval'    => 3,
                                'cookie_id'   => 'reduxsa_blast',
                            );

                            new reduxsaNewsflash( $this, $params );
                            $GLOBALS['reduxsa_notice_check'] = 1;
                        }
                    }
                }
                ...
                /**
                 * Loaded hook
                 * action 'reduxsa/loaded'
                 *
                 * @param  object $this ReduxSAFramework
                 */
                do_action( 'reduxsa/loaded', $this );
            }

            // __construct()

            private function set_reduxsa_content() {
                $upload_dir        = wp_upload_dir();
                self::$_upload_dir = $upload_dir['basedir'] . '/reduxsa/';
                self::$_upload_url = str_replace( array(
                    'https://',
                    'http://'
                ), '//', $upload_dir['baseurl'] . '/reduxsa/' );
            }
            ...
            private function set_default_args() {
                $this->args = array(
                    ...
                    'class'                     => '',
                    // Class that gets appended to all reduxsa-containers
                    'admin_bar'                 => true,
                    ...
                    'dev_mode'                  => true,
                    'templates_path'            => '',
                    // Path to the templates file for various ReduxSA elements
                    'ajax_save'                 => true,
            ...
            public function vc_fixes() {
                if ( reduxsa_helpers::isFieldInUse( $this, 'ace_editor' ) ) {
            ...
            public function network_admin_bar( $wp_admin_bar ) {
                ...
                $args = array(
                    ...
                    'meta'   => array( 'class' => 'reduxsa-network-admin' )
                );
            ...
            public function _update_check() {
                ...
                if ( ! isset ( $GLOBALS['reduxsa_update_check'] ) ) {
                    ReduxSA_Functions::updateCheck( self::$_version );
                    $GLOBALS['reduxsa_update_check'] = 1;
                }
            }

            public function _admin_notices() {
                ReduxSA_Admin_Notices::adminNotices( $this->admin_notices );
            }

            public function _dismiss_admin_notice() {
                ReduxSA_Admin_Notices::dismissAdminNotice();
            }
            ...
            private function _internationalization() {
                 ...
                 * filter 'reduxsa/textdomain/{opt_name}'
                 *
                 * @param string     The locale of the blog or from the 'locale' hook
                 * @param string     'reduxsa-framework'  text domain
                 */
                //                $locale = apply_filters( "reduxsa/textdomain/{$this->args['opt_name']}", get_locale(), 'reduxsa-framework' );
                //
                ...
                $basepath = apply_filters( "reduxsa/textdomain/basepath/{$this->args['opt_name']}", $basepath );

                $loaded = load_plugin_textdomain( 'reduxsa-framework', false, $basepath . 'languages');

                if ( !$loaded ){
                    $loaded = load_muplugin_textdomain( 'reduxsa-framework', $basepath . 'languages' );
                }

                if ( !$loaded ){
                    $loaded = load_theme_textdomain( 'reduxsa-framework', $basepath . 'languages' );
                }

                if ( ! $loaded ) {
                    $locale = apply_filters( 'plugin_locale', get_locale(), 'reduxsa-framework' );
                    $mofile = dirname( __FILE__ ) . '/languages/reduxsa-framework-' . $locale . '.mo';
                    load_textdomain( 'reduxsa-framework', $mofile );
                }
            }
            ...
            private function _tracking() {
                if ( file_exists( dirname( __FILE__ ) . '/inc/tracking.php' ) ) {
                    ...
                    $tracking = ReduxSA_Tracking::get_instance();
                    $tracking->load( $this );
                }
            }
            // _tracking()
            ...
            private function set_global_variable() {
                if ( $this->args['global_variable'] ) {
                    ...
                    /**
                     * filter 'reduxsa/options/{opt_name}/global_variable'
                     *
                     * @param array $value option value to set global_variable with
                     */
                    $GLOBALS[ $this->args['global_variable'] ] = apply_filters( "reduxsa/options/{$this->args['opt_name']}/global_variable", $this->options );
            ...
            public function set_options( $value = '' ) {
                ...
                if ( ! empty ( $value ) ) {
                    ...
                    //do_action( "reduxsa-saved-{$this->args['opt_name']}", $value ); // REMOVE
                    //do_action( "reduxsa/options/{$this->args['opt_name']}/saved", $value, $this->transients['changed_values'] );
                }
            }
            // set_options()
            ...
            public function get_options() {
                ...
                /**
                 * action 'reduxsa/options/{opt_name}/options'
                 *
                 * @param mixed $value option values
                 */
                $this->options = apply_filters( "reduxsa/options/{$this->args['opt_name']}/options", $this->options );
            ...
            public function get_wordpress_data( $type = false, $args = array() ) {
                ...
                //return $data;
                /**
                 * filter 'reduxsa/options/{opt_name}/wordpress_data/{type}/'
                 *
                 * @deprecated
                 *
                 * @param string $data
                 */
                $data = apply_filters( "reduxsa/options/{$this->args['opt_name']}/wordpress_data/$type/", $data ); // REMOVE LATER

                /**
                 * filter 'reduxsa/options/{opt_name}/data/{type}'
                 *
                 * @param string $data
                 */
                $data = apply_filters( "reduxsa/options/{$this->args['opt_name']}/data/$type", $data );
                ...
                if ( empty ( $data ) && ! empty ( $type ) ) {
                    ...
                    if ( ! empty ( $type ) && empty ( $data ) ) {
                        ...
                        if ( $type == "categories" || $type == "category" ) {
                            ...
                            /**
                             * filter 'reduxsa-font-icons'
                             *
                             * @deprecated
                             *
                             * @param array $font_icons array of elusive icon classes
                             */
                            $font_icons = apply_filters( 'reduxsa-font-icons', array() ); // REMOVE LATER
                             ...
                             * @param array $font_icons array of elusive icon classes
                             */
                            $font_icons = apply_filters( 'reduxsa/font-icons', $font_icons );

                            /**
                             * filter 'reduxsa/{opt_name}/field/font/icons'
                             *
                             * @deprecated
                             *
                             * @param array $font_icons array of elusive icon classes
                             */
                            $font_icons = apply_filters( "reduxsa/{$this->args['opt_name']}/field/font/icons", $font_icons );
            ...
            public function field_default_values( $field ) {
                ...
                if ( isset ( $field['default'] ) ) {
                    $this->options_defaults[ $field['id'] ] = apply_filters( "reduxsa/{$this->args['opt_name']}/field/{$field['type']}/defaults", $field['default'], $field );
            ...
            public function _default_values() {
                        ...
                        if ( isset ( $section['fields'] ) ) {
                            foreach ( $section['fields'] as $k => $field ) {
                                ...
                                if ( $field['type'] == "section" && isset ( $field['indent'] ) && $field['indent'] == "true" ) {
                                    ...
                                    $field['class'] .= " reduxsa-section-indent-start";
                ...
                /**
                 * filter 'reduxsa/options/{opt_name}/defaults'
                 *
                ...
                $this->options_defaults             = apply_filters( "reduxsa/options/{$this->args['opt_name']}/defaults", $this->options_defaults, $this->transients['changed_values'] );
            ...
            private function _default_cleanup() {
                ...
                if (ReduxSA_Helpers::isLocalHost() || ( ReduxSA_Helpers::isWpDebug() ) ) {
...
//                    }
            ...
            public function _admin_bar_menu() {
                ...
                if ( $menu ) {
                    ...
                    if ( isset ( $submenu[ $this->args["page_slug"] ] ) && is_array( $submenu[ $this->args["page_slug"] ] ) ) {
                        foreach ( $submenu[ $this->args["page_slug"] ] as $index => $reduxsa_options_submenu ) {
                            $subnodeargs = array(
                                'id'     => $this->args["page_slug"] . '_' . $index,
                                'title'  => $reduxsa_options_submenu[0],
                                'parent' => $this->args["page_slug"],
                                'href'   => admin_url( 'admin.php?page=' . $reduxsa_options_submenu[2] ),
                            );
                    ...
                    $nodeargs = array(
                        ...
                        'title' => $title,
                        // $theme_data->get( 'Name' ) . " " . __( 'Options', 'reduxsa-framework-demo' ),
            ...
            public function _enqueue_output() {
                        ...
                        foreach ( $section['fields'] as $fieldk => $field ) {
                            if ( isset ( $field['type'] ) && $field['type'] != "callback" ) {
                                ...
                                if ( ! class_exists( $field_class ) ) {
                                     ...
                                     */
                                    $class_file = apply_filters( "reduxsa/{$this->args['opt_name']}/field/class/{$field['type']}", self::$_dir . "inc/fields/{$field['type']}/field_{$field['type']}.php", $field );
                                ...
                                if ( ! empty ( $this->options[ $field['id'] ] ) && class_exists( $field_class ) && method_exists( $field_class, 'output' ) && $this->_can_output_css( $field ) ) {
                                    $field = apply_filters( "reduxsa/field/{$this->args['opt_name']}/output_css", $field );
                ...
                if ( ! empty ( $this->typography ) && ! empty ( $this->typography ) && filter_var( $this->args['output'], FILTER_VALIDATE_BOOLEAN ) ) {
                    ...
                    $typography = new ReduxSAFramework_typography ( null, null, $this );
                    ...
                    if ( $this->args['async_typography'] && ! empty ( $this->typography ) ) {
                        ...
                        wp_register_style( 'reduxsa-google-fonts-' . $this->args['opt_name'], $protocol . $typography->makeGoogleWebfontLink( $this->typography ), '', $version );
                        wp_enqueue_style( 'reduxsa-google-fonts-' . $this->args['opt_name'] );
                    }
                }
            }
            // _enqueue_output()
            ...
            public function _enqueue() {
                ...
                $enqueue = new reduxsaCoreEnqueue ( $this );
                $enqueue->init();
            }
            // _enqueue()
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    $dismiss = 'true';
                    $s       = __( 'Enable', 'reduxsa-framework' );

                    // Values for disabling hints.
                    if ( 'true' == $hint_status ) {
                        $dismiss = 'false';
                        $s       = __( 'Disable', 'reduxsa-framework' );
                    }

                    // Make URL
                    $url = '<a class="reduxsa_hint_status" href="?dismiss=' . $dismiss . '&amp;id=hints&amp;page=' . $curPage . '&amp;tab=' . $curTab . '">' . $s . ' hints</a>';

                    $event = __( 'moving the mouse over', 'reduxsa-framework' );
                    if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {
                        $event = __( 'clicking', 'reduxsa-framework' );
                    }

                    // Construct message
                    $msg = sprintf( __( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'reduxsa-framework' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;

                    // Construct hint tab
                    $tab = array(
                        'id'      => 'reduxsa-hint-tab',
                        'title'   => __( 'Hints', 'reduxsa-framework' ),
                        'content' => '<p>' . $msg . '</p>'
                    );
                ...
                if ( $this->args['help_sidebar'] != '' ) {
                    ...
                    // about hints.
                    if ( true == $this->show_hints ) {
                        $screen->set_help_sidebar( '<p><strong>ReduxSA Framework</strong><br/><br/>Hint Tooltip Preferences</p>' );
                    }
                }

                /**
                 * action 'reduxsa-load-page-{opt_name}'
                 *
                 * @deprecated
                 *
                 * @param object $screen WP_Screen
                 */
                do_action( "reduxsa-load-page-{$this->args['opt_name']}", $screen ); // REMOVE

                /**
                 * action 'reduxsa/page/{opt_name}/load'
                 *
                 * @param object $screen WP_Screen
                 */
                do_action( "reduxsa/page/{$this->args['opt_name']}/load", $screen );
            }
            // _load_page()

            /**
             * Do action reduxsa-admin-head for options page
             *
             * @since       1.0.0
             * @access      public
             * @return      void
            ...
            public function admin_head() {
                /**
                 * action 'reduxsa-admin-head-{opt_name}'
                 *
                 * @deprecated
                 *
                 * @param  object $this ReduxSAFramework
                 */
                do_action( "reduxsa-admin-head-{$this->args['opt_name']}", $this ); // REMOVE

                /**
                 * action 'reduxsa/page/{opt_name}/header'
                 *
                 * @param  object $this ReduxSAFramework
                 */
                do_action( "reduxsa/page/{$this->args['opt_name']}/header", $this );
            }
            // admin_head()
            ...
            private function get_default_output_string( $field ) {
                ...
                if ( ! empty ( $default_output ) ) {
                    $default_output = __( 'Default', 'reduxsa-framework' ) . ": " . substr( $default_output, 0, - 2 );
                }
            ...
            public function get_header_html( $field ) {
                ...
                if ( isset ( $field['title'] ) && isset ( $field['type'] ) && $field['type'] !== "info" && $field['type'] !== "section" ) {
                    ...
                    if ( isset ( $field['hint'] ) && ! '' == $field['hint'] ) {
                        ...
                        $hint = apply_filters( 'reduxsa/hints/html', $hint, $field, $this->args );
                        ...
                        if ( 'true' == $metaVal || empty ( $metaVal ) && empty( $hint ) ) {
                            ...
                            $hint = '<div class="reduxsa-hint-qtip" style="float:' . $this->args['hints']['icon_position'] . '; font-size: ' . $size . '; color:' . $hint_color . '; cursor: ' . $pointer . ';" qtip-title="' . $titleParam . '" qtip-content="' . $contentParam . '">&nbsp;<i class="' . ( isset( $this->args['hints']['icon'] ) ? $this->args['hints']['icon'] : '' ) . '"></i></div>';
                        }
                    }
                ...
                if ( ! empty ( $th ) ) {
                    $th = '<div class="reduxsa_field_th">' . $th . '</div>';
                }
            ...
            public function _register_settings() {
                ...
                foreach ( $this->sections as $k => $section ) {
                    ...
                    /**
                     * filter 'reduxsa-section-{index}-modifier-{opt_name}'
                     *
                     * @param array $section section configuration
                     */
                    $section = apply_filters( "reduxsa-section-{$k}-modifier-{$this->args['opt_name']}", $section );

                    /**
                     * filter 'reduxsa/options/{opt_name}/section/{section.id}'
                     *
                     * @param array $section section configuration
                     */
                    if ( isset ( $section['id'] ) ) {
                        $section = apply_filters( "reduxsa/options/{$this->args['opt_name']}/section/{$section['id']}", $section );
                    }
                    ...
                    if ( isset ( $section['fields'] ) ) {
                        foreach ( $section['fields'] as $fieldk => $field ) {
                            ...
                            /**
                             * filter 'reduxsa/options/{opt_name}/field/{field.id}'
                             *
                             * @param array $field field config
                             */
                            $field = apply_filters( "reduxsa/options/{$this->args['opt_name']}/field/{$field['id']}/register", $field );
                            ...
                            /**
                             * filter 'reduxsa-field-{field.id}modifier-{opt_name}'
                             *
                             * @deprecated
                             *
                             * @param array $field field config
                             */
                            $field = apply_filters( "reduxsa-field-{$field['id']}modifier-{$this->args['opt_name']}", $field ); // REMOVE LATER
                             ...
                             * @param array $field field config
                             */
                            $field = apply_filters( "reduxsa/options/{$this->args['opt_name']}/field/{$field['id']}", $field );
                            ...
                            /**
                             * action 'reduxsa/options/{opt_name}/field/field.type}/register'
                             */
                            do_action( "reduxsa/options/{$this->args['opt_name']}/field/{$field['type']}/register", $field );
                            ...
                            if ( ! $display || isset ( $this->no_panel_section[ $k ] ) ) {
                                ...
                                if ( isset ( $field['hidden'] ) && $field['hidden'] ) {
                                    $field['label_for'] = 'reduxsa_hide_field';
                                }
                                if ( $this->args['options_api'] == true ) {
                                    add_settings_field(
                ...
                /**
                 * action 'reduxsa-register-settings-{opt_name}'
                 *
                 * @deprecated
                 */
                do_action( "reduxsa-register-settings-{$this->args['opt_name']}" ); // REMOVE

                /**
                 * action 'reduxsa/options/{opt_name}/register'
                 *
                 * @param array option sections
                 */
                do_action( "reduxsa/options/{$this->args['opt_name']}/register", $this->sections );
                ...
                if ( isset ( $this->transients['run_compiler'] ) && $this->transients['run_compiler'] ) {
                    ...
                    /**
                     * action 'reduxsa-compiler-{opt_name}'
                     *
                     * @deprecated
                     *
                     * @param array  options
                     ...
                     */
                    do_action( "reduxsa-compiler-{$this->args['opt_name']}", $this->options, $this->compilerCSS, $this->transients['changed_values'] ); // REMOVE

                    /**
                     * action 'reduxsa/options/{opt_name}a'
                     *
                     * @param array  options
                     * @param string CSS that get sent to the compiler hook
                     */
                    do_action( "reduxsa/options/{$this->args['opt_name']}/compiler", $this->options, $this->compilerCSS, $this->transients['changed_values'] );

                    /**
                     * action 'reduxsa/options/{opt_name}/compiler/advanced'
                     *
                     * @param array  options
                     * @param string CSS that get sent to the compiler hook, which sends the full ReduxSA object
                     */
                    do_action( "reduxsa/options/{$this->args['opt_name']}/compiler/advanced", $this );
            ...
            private function _register_extensions() {
                ...
                /**
                 * action 'reduxsa/extensions/before'
                 *
                 * @param object $this ReduxSAFramework
                 */
                do_action( "reduxsa/extensions/before", $this );

                /**
                 * action 'reduxsa/extensions/{opt_name}/before'
                 *
                 * @param object $this ReduxSAFramework
                 */
                do_action( "reduxsa/extensions/{$this->args['opt_name']}/before", $this );

                if ( isset( $this->old_opt_name ) ) {
                    do_action( "reduxsa/extensions/{$this->old_opt_name}/before", $this );
                }
                ...
                foreach ( $folders as $folder ) {
                    ...
                    $extension_class = 'ReduxSAFramework_Extension_' . $folder;

                    /**
                     * filter 'reduxsa-extensionclass-load'
                     *
                     * @deprecated
                     *
                     ...
                     */
                    $class_file = apply_filters( "reduxsa-extensionclass-load", "$path/$folder/extension_{$folder}.php", $extension_class ); // REMOVE LATER

                    /**
                     * filter 'reduxsa/extension/{opt_name}/{folder}'
                     *
                     * @param        string                    extension class file path
                     * @param string $extension_class          extension class name
                     */
                    $class_file = apply_filters( "reduxsa/extension/{$this->args['opt_name']}/$folder", "$path/$folder/extension_{$folder}.php", $class_file );
                ...
                /**
                 * action 'reduxsa-register-extensions-{opt_name}'
                 *
                 * @deprecated
                 *
                 * @param object $this ReduxSAFramework
                 */
                do_action( "reduxsa-register-extensions-{$this->args['opt_name']}", $this ); // REMOVE

                /**
                 * action 'reduxsa/extensions/{opt_name}'
                 *
                 * @param object $this ReduxSAFramework
                 */
                do_action( "reduxsa/extensions/{$this->args['opt_name']}", $this );

                if ( isset( $this->old_opt_name ) && ! empty( $this->old_opt_name ) ) {
                    do_action( "reduxsa/extensions/{$this->old_opt_name}", $this );
                }
            }
            ...
            public function _validate_options( $plugin_options ) {
                ...
                if ( isset ( $plugin_options['reduxsa-no_panel'] ) ) {
                    $keys = explode( '|', $plugin_options['reduxsa-no_panel'] );
                    foreach ( $keys as $key ) {
                        $plugin_options[ $key ] = $this->options[ $key ];
                    }
                    if ( isset ( $plugin_options['reduxsa-no_panel'] ) ) {
                        unset ( $plugin_options['reduxsa-no_panel'] );
                    }
                }
                ...
                if ( ( isset( $plugin_options['import_code'] ) && ! empty( $plugin_options['import_code'] ) ) || ( isset( $plugin_options['import_link'] ) && ! empty( $plugin_options['import_link'] ) ) ) {
                    ...
                    if ( ! empty ( $imported_options ) && is_array( $imported_options ) && isset ( $imported_options['reduxsa-backup'] ) && $imported_options['reduxsa-backup'] == '1' ) {
                         ...
                         *
                         * @param  &array [&$plugin_options, reduxsa_options]
                         */
                        do_action_ref_array( "reduxsa/options/{$this->args['opt_name']}/import", array(
                            &$plugin_options,
                            $imported_options,
                            $this->transients['changed_values']
                        ) );

                        setcookie( 'reduxsa_current_tab', '', 1, '/', $time + 1000, "/" );
                ...
                if ( ! empty ( $plugin_options['defaults'] ) ) {
                    ...
                    /**
                     * apply_filters 'reduxsa/validate/{opt_name}/defaults'
                     *
                     * @param  &array [ $this->options_defaults, $plugin_options]
                     */
                    $plugin_options = apply_filters( "reduxsa/validate/{$this->args['opt_name']}/defaults", $this->options_defaults );
                    ...
                    //setcookie('reduxsa-compiler-' . $this->args['opt_name'], 1, time() + 1000, "/");
                    //setcookie("reduxsa-saved-{$this->args['opt_name']}", 'defaults', time() + 1000, "/");
                ...
                if ( ! empty ( $plugin_options['defaults-section'] ) ) {
                    if ( isset ( $plugin_options['reduxsa-section'] ) && isset ( $this->sections[ $plugin_options['reduxsa-section'] ]['fields'] ) ) {
                        /**
                         * apply_filters 'reduxsa/validate/{opt_name}/defaults_section'
                         *
                         * @param  &array [ $this->options_defaults, $plugin_options]
                         */
                        foreach ( $this->sections[ $plugin_options['reduxsa-section'] ]['fields'] as $field ) {
                        ...
                        $plugin_options = apply_filters( "reduxsa/validate/{$this->args['opt_name']}/defaults_section", $plugin_options );
                    }
                    ...
                    //setcookie("reduxsa-saved-{$this->args['opt_name']}", 'defaults_section', time() + 1000, "/");
                    unset ( $plugin_options['defaults'], $plugin_options['defaults_section'], $plugin_options['import'], $plugin_options['import_code'], $plugin_options['import_link'], $plugin_options['compiler'], $plugin_options['reduxsa-section'] );
                ...
                /**
                 * apply_filters 'reduxsa/validate/{opt_name}/before_validation'
                 *
                 * @param  &array [&$plugin_options, reduxsa_options]
                 */
                $plugin_options = apply_filters( "reduxsa/validate/{$this->args['opt_name']}/before_validation", $plugin_options, $this->options );
                ...
                /**
                 * action 'reduxsa-validate-{opt_name}'
                 *
                 * @deprecated
                 *
                 * @param  &array [&$plugin_options, reduxsa_options]
                 */
                do_action_ref_array( "reduxsa-validate-{$this->args['opt_name']}", array(
                    &$plugin_options,
                    $this->options
                ) ); // REMOVE
                ...
                /**
                 * action 'reduxsa/options/{opt_name}/validate'
                 *
                 * @param  &array [&$plugin_options, reduxsa_options]
                 */
                do_action_ref_array( "reduxsa/options/{$this->args['opt_name']}/validate", array(
                    &$plugin_options,
                    $this->options,
                    $this->transients['changed_values']
                ) );
            ...
            public function ajax_save() {
                if ( ! wp_verify_nonce( $_REQUEST['nonce'], "reduxsa_ajax_nonce" . $this->args['opt_name'] ) ) {
                    echo json_encode( array(
                        'status' => __( 'Invalid security credential.  Please reload the page and try again.', 'reduxsa-framework' ),
                        'action' => ''
                    ) );
                ...
                if ( ! self::current_user_can( $this->args['page_permissions'] ) ) {
                    echo json_encode( array(
                        'status' => __( 'Invalid user capability.  Please reload the page and try again.', 'reduxsa-framework' ),
                        'action' => ''
                    ) );
                ...
                $reduxsa = ReduxSAFrameworkInstances::get_instance( $_POST['opt_name'] );

                if ( ! empty ( $_POST['data'] ) && ! empty ( $reduxsa->args['opt_name'] ) ) {
                    ...
                    $values = $this->reduxsa_parse_str( $_POST['data'] );

                    $values = $values[ $reduxsa->args['opt_name'] ];
                    ...
                    if ( ! empty ( $values ) ) {
                        ...
                        try {
                            if ( isset ( $reduxsa->validation_ran ) ) {
                                unset ( $reduxsa->validation_ran );
                            }
                            $reduxsa->set_options( $reduxsa->_validate_options( $values ) );
                            ...
                            require_once 'core/enqueue.php';
                            $enqueue = new reduxsaCoreEnqueue ( $reduxsa );
                            $enqueue->get_warnings_and_errors_array();

                            $return_array = array(
                                'status'   => 'success',
                                'options'  => $reduxsa->options,
                                'errors'   => isset ( $reduxsa->localize_data['errors'] ) ? $reduxsa->localize_data['errors'] : null,
                                'warnings' => isset ( $reduxsa->localize_data['warnings'] ) ? $reduxsa->localize_data['warnings'] : null,
                            );
                        ...
                        echo json_encode( array( 'status' => __( 'Your panel has no fields. Nothing to save.', 'reduxsa-framework' ) ) );
                    }
                }
                if ( isset ( $this->transients['run_compiler'] ) && $this->transients['run_compiler'] ) {
                    ...
                    try {
                        ...
                        /**
                         * action 'reduxsa/options/{opt_name}/compiler'
                         *
                         * @param array  options
                         * @param string CSS that get sent to the compiler hook
                         */
                ...
                if ( isset( $return_array ) ) {
                    if ( $return_array['status'] == "success" ) {
                        ...
                        $panel = new reduxsaCorePanel ( $reduxsa );
                        ob_start();
                    ...
                    echo json_encode( apply_filters( "reduxsa/options/{$this->args['opt_name']}/ajax_save/response", $return_array ) );
                }
...
//                            }
                            ...
                            if ( isset ( $field['validate'] ) ) {
                                $validate = 'ReduxSA_Validation_' . $field['validate'];

                                if ( ! class_exists( $validate ) ) {
                                    /**
                                     * filter 'reduxsa-validateclass-load'
                                     *
                                     * @deprecated
                                     *
                                     ...
                                     */
                                    $class_file = apply_filters( "reduxsa-validateclass-load", self::$_dir . "inc/validation/{$field['validate']}/validation_{$field['validate']}.php", $validate ); // REMOVE LATER

                                    /**
                                     * filter 'reduxsa/validate/{opt_name}/class/{field.validate}'
                                     *
                                     * @param        string                validation class file path
                                     * @param string $class_file           validation class file path
                                     */
                                    $class_file = apply_filters( "reduxsa/validate/{$this->args['opt_name']}/class/{$field['validate']}", self::$_dir . "inc/validation/{$field['validate']}/validation_{$field['validate']}.php", $class_file );
            ...
            public function section_menu( $k, $section, $suffix = "", $sections = array() ) {
                ...
                if ( isset ( $section['type'] ) && $section['type'] == "divide" ) {
                    ...
                    $string .= '<li id="' . esc_attr( $k . $suffix ) . '_section_group_li" class="reduxsa-group-tab-link-li' . esc_attr( $hide_section ) . esc_attr( $section['class'] ) . esc_attr( $subsectionsClass ) . '">';
                    $string .= '<a href="javascript:void(0);" id="' . esc_attr( $k . $suffix ) . '_section_group_li_a" class="reduxsa-group-tab-link-a" data-key="' . esc_attr( $k ) . '" data-rel="' . esc_attr( $k . $suffix ) . '">' . $extra_icon . $icon . '<span class="group_title">' . wp_kses_post( $section['title'] ) . '</span></a>';
                    ...
                    if ( $subsections ) {
                        ...
                        while ( $doLoop ) {
                            ...
                            if ( count( $sections ) < $nextK || ! isset ( $sections[ $nextK ] ) || ! isset ( $sections[ $nextK ]['subsection'] ) || $sections[ $nextK ]['subsection'] != true ) {
                                ...
                                $string .= '<li id="' . esc_attr( $nextK . $suffix ) . '_section_group_li" class="reduxsa-group-tab-link-li ' . esc_attr( $hide_sub ) . esc_attr( $section[ $nextK ]['class'] ) . ( $icon ? ' hasIcon' : '' ) . '">';
                                $string .= '<a href="javascript:void(0);" id="' . esc_attr( $nextK . $suffix ) . '_section_group_li_a" class="reduxsa-group-tab-link-a" data-key="' . esc_attr( $nextK ) . '" data-rel="' . esc_attr( $nextK . $suffix ) . '">' . $icon . '<span class="group_title">' . wp_kses_post( $sections[ $nextK ]['title'] ) . '</span></a>';
                                $string .= '</li>';
                            }
                        }
            ...
            public function generate_panel() {
                require_once 'core/panel.php';
                $panel = new reduxsaCorePanel ( $this );
                $panel->init();
                $this->set_transients();
            }
            ...
            public function _section_desc( $section ) {
                ...
                if ( isset ( $this->sections[ $id ]['desc'] ) && ! empty ( $this->sections[ $id ]['desc'] ) ) {
                    echo '<div class="reduxsa-section-desc">' . $this->sections[ $id ]['desc'] . '</div>';
                }
            }
            ...
            public function _field_input( $field, $v = null ) {
                ...
                if ( isset ( $field['callback'] ) && ( is_callable( $field['callback'] ) || ( is_string( $field['callback'] ) && function_exists( $field['callback'] ) ) ) ) {
                    ...
                    /**
                     * action 'reduxsa-before-field-{opt_name}'
                     *
                     * @deprecated
                     *
                     ...
                     */
                    do_action( "reduxsa-before-field-{$this->args['opt_name']}", $field, $value ); // REMOVE

                    /**
                     * action 'reduxsa/field/{opt_name}/{field.type}/callback/before'
                     *
                     * @param array  $field field data
                     * @param string $value field.id
                     */
                    do_action_ref_array( "reduxsa/field/{$this->args['opt_name']}/{$field['type']}/callback/before", array(
                        &$field,
                        &$value
                    ) );

                    /**
                     * action 'reduxsa/field/{opt_name}/callback/before'
                     *
                     * @param array  $field field data
                     * @param string $value field.id
                     */
                    do_action_ref_array( "reduxsa/field/{$this->args['opt_name']}/callback/before", array(
                        &$field,
                        &$value
                    ) );
                    ...
                    /**
                     * action 'reduxsa-after-field-{opt_name}'
                     *
                     * @deprecated
                     *
                     * @param array  $field field data
                     * @param string $value field.id
                     */
                    do_action( "reduxsa-after-field-{$this->args['opt_name']}", $field, $value ); // REMOVE

                    /**
                     * action 'reduxsa/field/{opt_name}/{field.type}/callback/after'
                     *
                     * @param array  $field field data
                     * @param string $value field.id
                     */
                    do_action_ref_array( "reduxsa/field/{$this->args['opt_name']}/{$field['type']}/callback/after", array(
                        &$field,
                        &$value
                    ) );

                    /**
                     * action 'reduxsa/field/{opt_name}/callback/after'
                     *
                     * @param array  $field field data
                     * @param string $value field.id
                     */
                    do_action_ref_array( "reduxsa/field/{$this->args['opt_name']}/callback/after", array(
                        &$field,
                        &$value
                    ) );
                ...
                if ( isset ( $field['type'] ) ) {
                    ...
                    if ( ! class_exists( $field_class ) ) {
                        //                    $class_file = apply_filters( 'reduxsa/field/class/'.$field['type'], self::$_dir . 'inc/fields/' . $field['type'] . '/field_' . $field['type'] . '.php', $field ); // REMOVE
                        /**
                         * filter 'reduxsa/{opt_name}/field/class/{field.type}'
                         *
                    ...
                    if ( class_exists( $field_class ) ) {
                        ...
                        /**
                         * action 'reduxsa/field/{opt_name}/{field.type}/render/before'
                         *
                         * @param array  $field field data
                         * @param string $value field id
                         */
                        do_action_ref_array( "reduxsa/field/{$this->args['opt_name']}/{$field['type']}/render/before", array(
                            &$field,
                            &$value
                        ) );

                        /**
                         * action 'reduxsa/field/{$this->args['opt_name']}/render/before'
                         *
                         * @param array  $field field data
                         * @param string $value field id
                         */
                        do_action_ref_array( "reduxsa/field/{$this->args['opt_name']}/render/before", array(
                            &$field,
                            &$value
                        ) );
                     ...
                     */
                         ...
                         * @param array $field        field data
                         */
                        $_render = apply_filters( "reduxsa-field-{$this->args['opt_name']}", ob_get_contents(), $field ); // REMOVE

                        /**
                         * filter 'reduxsa/field/{opt_name}/{field.type}/render/after'
                         *
                         * @param       string        rendered field markup
                         * @param array $field        field data
                         */
                        $_render = apply_filters( "reduxsa/field/{$this->args['opt_name']}/{$field['type']}/render/after", $_render, $field );

                        /**
                         * filter 'reduxsa/field/{opt_name}/render/after'
                         *
                         * @param       string        rendered field markup
                         * @param array $field        field data
                         */
                        $_render = apply_filters( "reduxsa/field/{$this->args['opt_name']}/render/after", $_render, $field );
                        ...
                        /**
                         * action 'reduxsa/field/{opt_name}/{field.type}/fieldset/before/{opt_name}'
                         *
                         * @param array  $field field data
                         * @param string $value field id
                         */
                        do_action_ref_array( "reduxsa/field/{$this->args['opt_name']}/{$field['type']}/fieldset/before/{$this->args['opt_name']}", array(
                            &$field,
                            &$value
                        ) );

                        /**
                         * action 'reduxsa/field/{opt_name}/fieldset/before/{opt_name}'
                         *
                         * @param array  $field field data
                         * @param string $value field id
                         */
                        do_action_ref_array( "reduxsa/field/{$this->args['opt_name']}/fieldset/before/{$this->args['opt_name']}", array(
                            &$field,
                            &$value
                        ) );
                        ...
                        if ( isset( $field['full_width'] ) && $field['full_width'] == true ) {
                            $class_string .= "reduxsa_remove_th";
                        }
                        ...
                        echo '<fieldset id="' . $this->args['opt_name'] . '-' . $field['id'] . '" class="' . $hidden . 'reduxsa-field-container reduxsa-field reduxsa-field-init reduxsa-container-' . $field['type'] . ' ' . $class_string . '" data-id="' . $field['id'] . '" ' . $data_string . ' data-type="' . $field['type'] . '">';
                        //}
                        ...
                        /**
                         * action 'reduxsa/field/{opt_name}/{field.type}/fieldset/after/{opt_name}'
                         *
                         * @param array  $field field data
                         * @param string $value field id
                         */
                        do_action_ref_array( "reduxsa/field/{$this->args['opt_name']}/{$field['type']}/fieldset/after/{$this->args['opt_name']}", array(
                            &$field,
                            &$value
                        ) );

                        /**
                         * action 'reduxsa/field/{opt_name}/fieldset/after/{opt_name}'
                         *
                         * @param array  $field field data
                         * @param string $value field id
                         */
                        do_action_ref_array( "reduxsa/field/{$this->args['opt_name']}/fieldset/after/{$this->args['opt_name']}", array(
                            &$field,
                            &$value
                        ) );
                    }
            ...
            public function _can_output_css( $field ) {
                ...
                $field = apply_filters( "reduxsa/field/{$this->args['opt_name']}/_can_output_css", $field );
             ...
             * @return  array $result
             */
            function reduxsa_parse_str( $string ) {
                if ( '' == $string ) {
                    return false;
                }
                ...
                foreach ( $pairs as $key => $pair ) {
                    ...
                    if ( ! isset( $result[ $k ] ) ) {
                        ...
                        $result[ $k ] = $this->reduxsa_array_merge_recursive_distinct( $result[ $k ], $params[ $k ] );
                    }
                }
             ...
             * @return  array $merged
             */
            function reduxsa_array_merge_recursive_distinct( array $array1, array $array2 ) {
                $merged = $array1;

                foreach ( $array2 as $key => $value ) {
                    if ( is_array( $value ) && isset( $merged[ $key ] ) && is_array( $merged[ $key ] ) ) {
                        $merged[ $key ] = $this->reduxsa_array_merge_recursive_distinct( $merged[ $key ], $value );
                    ...
                    if ( ! empty( $this->args['admin_bar_links'] ) ) {
                        foreach ( $this->args['admin_bar_links'] as $idx => $arr ) {
                            if ( is_array( $arr ) && ! empty( $arr ) ) {
                                foreach ( $arr as $x => $y ) {
                                    if ( strpos( strtolower( $y ), 'reduxsa' ) !== false ) {
                                        $msg = __( '<strong>ReduxSA Framework Notice: </strong>There are references to the ReduxSA Framework support site in your config\'s <code>admin_bar_links</code> argument.  This is sample data.  Please change or remove this data before shipping your product.', 'reduxsa-framework' );
                    ...
                    if ( ! empty( $this->args['share_icons'] ) ) {
                        foreach ( $this->args['share_icons'] as $idx => $arr ) {
                            if ( is_array( $arr ) && ! empty( $arr ) ) {
                                foreach ( $arr as $x => $y ) {
                                    if ( strpos( strtolower( $y ), 'reduxsa' ) !== false ) {
                                        $msg = __( '<strong>ReduxSA Framework Notice: </strong>There are references to the ReduxSA Framework support site in your config\'s <code>share_icons</code> argument.  This is sample data.  Please change or remove this data before shipping your product.', 'reduxsa-framework' );
         ...
         * @param null
         */
        do_action( 'reduxsa/init', ReduxSAFramework::init() );
    } // class_exists('ReduxSAFramework')
