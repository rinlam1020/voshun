/*global reduxsa_change, wp, reduxsa, libFilter */
...
(function($){
    ...
    reduxsa.field_objects.media   = reduxsa.field_objects.media || {};
    ...
    reduxsa.field_objects.media.init = function( selector ) {
        if ( !selector ) {
            selector = $( document ).find( ".reduxsa-group-tab:visible" ).find( '.reduxsa-container-media:visible' );
        }
        $( selector ).each(
            function() {
                var el = $( this );
                ...
                el.find( '.remove-image, .remove-file' ).unbind( 'click' ).on(
                    'click', function() {
                        reduxsa.field_objects.media.removeFile( $( this ).parents( 'fieldset.reduxsa-field:first' ) );
                    }
                );
                // Upload media button
                el.find( '.media_upload_button' ).unbind().on(
                    'click', function( event ) {
                        reduxsa.field_objects.media.addFile( event, $( this ).parents( 'fieldset.reduxsa-field:first' ) );
                    }
                );
            }
        );
    ...
    reduxsa.field_objects.media.addFile = function( event, selector ) {
        ...
        frame.on(
            'select', function() {
                ...
                if ( typeof reduxsa.field_objects.media === 'undefined' || typeof reduxsa.field_objects.media === undefined ) {
                    reduxsa.field_objects.media = {};
                }
                ...
                selector.find( '.reduxsa-background-properties' ).slideDown();
            }
        );
    ...
    reduxsa.field_objects.media.removeFile = function( selector ) {
        ...
        reduxsa_change( $( selector ).find( '.upload-id' ) );
        selector.find( '.reduxsa-background-properties' ).hide();
