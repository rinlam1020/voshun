/* global jQuery, document, reduxsa, reduxsa_change, setting */
...
(function( $ ) {  //This functions first parameter is named $
    ...
    reduxsa.customizer = reduxsa.customizer || {};

    $( document ).ready(
        function() {
            reduxsa.customizer.init();
        }
    );
    reduxsa.customizer.init = function() {
        $( 'body' ).addClass( reduxsa_customizer.body_class );
        $( '.accordion-section.reduxsa-section, .accordion-section.reduxsa-panel, .accordion-section-title' ).click(
            function() {
                $.reduxsa.initFields();
            }
        );

        reduxsa.args.disable_save_warn = true;
        var reduxsaChange = reduxsa_change;
        reduxsa_change = function( variable ) {
            variable = $( variable );
            reduxsaChange.apply( this, arguments );
            reduxsa.customizer.save( variable )
        };

        var reduxsa_initFields = $.reduxsa.initFields;
        $.reduxsa.initFiles = function() {
            reduxsa_initFields();
        }
    };

    reduxsa.customizer.save = function( $obj ) {
        var $parent = $obj.hasClass( 'reduxsa-field' ) ? $obj : $obj.parents( '.reduxsa-field-container:first' );
        reduxsa.customizer.inputSave( $parent );
    };
    reduxsa.customizer.inputSave = function( $parent ) {

        if ( !$parent.hasClass( 'reduxsa-field-container' ) ) {
            $parent = $parent.parents( '[class^="reduxsa-field-container"]' );
        }

        var $id = $parent.parent().find( '.reduxsa-customizer-input' ).data( 'id' );

        if ( !$id ) {
            $parent = $parent.parents( '.reduxsa-container-repeater:first' );
            var $id = $parent.parent().find( '.reduxsa-customizer-input' ).data( 'id' );
        }
        ...
        var $key = $parent.parent().find( '.reduxsa-customizer-input' ).data( 'key' );
        if ( $nData[$key] ) {
            $nData = $nData[$key];
        }
