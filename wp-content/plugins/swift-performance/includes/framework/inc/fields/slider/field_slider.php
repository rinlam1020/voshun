...
if ( ! class_exists( 'ReduxSAFramework_slider' ) ) {
    class ReduxSAFramework_slider {
        ...
        function enqueue() {
            ...
            wp_enqueue_style(
                'reduxsa-nouislider-css',
                ReduxSAFramework::$_url . 'inc/fields/slider/vendor/nouislider/reduxsa.jquery.nouislider.css',
                array(),
                '5.0.0',
                'all'
            );
            ...
            wp_register_script(
                'reduxsa-nouislider-js',
                ReduxSAFramework::$_url . 'inc/fields/slider/vendor/nouislider/reduxsa.jquery.nouislider' . $min . '.js',
                array( 'jquery' ),
                '5.0.0',
                true
            );
            ...
            wp_enqueue_script(
                'reduxsa-field-slider-js',
                ReduxSAFramework::$_url . 'inc/fields/slider/field_slider' . $min . '.js',
                array( 'jquery', 'reduxsa-nouislider-js', 'reduxsa-js', 'select2-js' ),
                time(),
                true
            );
            ...
            if ($this->parent->args['dev_mode']) {
                wp_enqueue_style(
                    'reduxsa-field-slider-css',
                    ReduxSAFramework::$_url . 'inc/fields/slider/field_slider.css',
                    array(),
                    time(),
                    'all'
                );
        ...
        function render() {
            ...
            if ( $this->display_text == $this->field['display_value'] ) {
                ...
                echo '<input type="text"
                         ...
                         id="' . $idOne . '"
                         value="' . $valOne . '"
                         class="reduxsa-slider-input reduxsa-slider-input-one-' . $fieldID . ' ' . $this->field['class'] . '"/>';
            ...
            // LABEL output
                ...
                echo '<div class="reduxsa-slider-label' . $labelNum . '"
                       id="reduxsa-slider-label-one-' . $fieldID . '"
                       name="' . $nameOne . '">
                  </div>';
            ...
            // SELECT output
                ...
                echo '<select class="reduxsa-slider-select-one reduxsa-slider-select-one-' . $fieldID . ' ' . $this->field['class'] . '"
                          name="' . $nameOne . '"
                          id="' . $idOne . '">
                 </select>';
            }
            ...
            echo 
            '<div
                class="reduxsa-slider-container ' . $this->field['class'] . '"
                id="' . $fieldID . '"
                data-id="' . $fieldID . '"
            ...
            if ( true == $twoHandles ) {
                ...
                if ( true == $showInput ) {
                    echo '<input type="text"
                             ...
                             id="' . $idTwo . '"
                             value="' . $valTwo . '"
                             class="reduxsa-slider-input reduxsa-slider-input-two-' . $fieldID . ' ' . $this->field['class'] . '"/>';
                }

                // LABEL
                if ( true == $showLabel ) {
                    echo '<div class="reduxsa-slider-label-two"
                           id="reduxsa-slider-label-two-' . $fieldID . '"
                           name="' . $nameTwo . '">
                      </div>';
                }
                ...
                // SELECT
                if ( true == $showSelect ) {
                    echo '<select class="reduxsa-slider-select-two reduxsa-slider-select-two-' . $fieldID . ' ' . $this->field['class'] . '"
                              name="' . $nameTwo . '"
                              id="' . $idTwo . '">
                     </select>';
            ...
            if ( $this->display_none == $this->field['display_value'] || $this->display_label == $this->field['display_value'] ) {
                echo '<input type="hidden"
                         class="reduxsa-slider-value-one-' . $fieldID . ' ' . $this->field['class'] . '"
                         name="' . $nameOne . '"
                         id="' . $idOne . '"
                         value="' . $valOne . '"/>';
                ...
                if ( true == $twoHandles ) {
                    echo '<input type="hidden"
                             class="reduxsa-slider-value-two-' . $fieldID . ' ' . $this->field['class'] . '"
                             name="' . $nameTwo . '"
                             id="' . $idTwo . '"
                             value="' . $valTwo . '"/>';
                }
