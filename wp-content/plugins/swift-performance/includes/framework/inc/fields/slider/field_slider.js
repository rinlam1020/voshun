...
(function( $ ) {
    ...
    reduxsa.field_objects.slider = reduxsa.field_objects.slider || {};
    ...
    reduxsa.field_objects.slider.init = function( selector ) {

        if ( !selector ) {
            selector = $( document ).find( ".reduxsa-group-tab:visible" ).find( '.reduxsa-container-slider:visible' );
        }
        ...
        $( selector ).each(
            function() {
                ...
                el.find( 'div.reduxsa-slider-container' ).each(
                    function() {
                        ...
                        if ( displayValue == DISPLAY_TEXT ) {
                            defClassOne = el.find( '.reduxsa-slider-input-one-' + mainID );
                            defClassTwo = el.find( '.reduxsa-slider-input-two-' + mainID );

                            inputOne = defClassOne;
                            inputTwo = defClassTwo;
                        } else if ( displayValue == DISPLAY_SELECT ) {
                            defClassOne = el.find( '.reduxsa-slider-select-one-' + mainID );
                            defClassTwo = el.find( '.reduxsa-slider-select-two-' + mainID );

                            reduxsa.field_objects.slider.loadSelect( defClassOne, minVal, maxVal, resVal, stepVal );

                            if ( handles === 2 ) {
                                reduxsa.field_objects.slider.loadSelect( defClassTwo, minVal, maxVal, resVal, stepVal );
                            }

                        } else if ( displayValue == DISPLAY_LABEL ) {
                            defClassOne = el.find( '#reduxsa-slider-label-one-' + mainID );
                            defClassTwo = el.find( '#reduxsa-slider-label-two-' + mainID );
                        } else if ( displayValue == DISPLAY_NONE ) {
                            defClassOne = el.find( '.reduxsa-slider-value-one-' + mainID );
                            defClassTwo = el.find( '.reduxsa-slider-value-two-' + mainID );
                        }
                                ...
                                slide: function() {
                                    if ( displayValue == DISPLAY_LABEL ) {
                                        if ( handles === 2 ) {
                                            ...
                                            el.find( 'input.reduxsa-slider-value-one-' + mainID ).attr(
                                                'value', inpSliderVal[0]
                                            );
                                            el.find( 'input.reduxsa-slider-value-two-' + mainID ).attr(
                                    ...
                                    if ( displayValue == DISPLAY_SELECT ) {
                                        if ( handles === 2 ) {
                                            el.find( '.reduxsa-slider-select-one' ).select2( 'val', slider.val()[0] );
                                            el.find( '.reduxsa-slider-select-two' ).select2( 'val', slider.val()[1] );
                                        } else {
                                            el.find( '.reduxsa-slider-select-one' ).select2( 'val', slider.val() );
                                        }
                                    }

                                    reduxsa_change( $( this ).parents( '.reduxsa-field-container:first' ).find( 'input' ) );
                                }
                            }
                        );
                ...
                el.find( 'select.reduxsa-slider-select-one, select.reduxsa-slider-select-two' ).select2( default_params );
    ...
    reduxsa.field_objects.slider.isFloat = function( mixed_var ) {
        return +mixed_var === mixed_var && (!(isFinite( mixed_var ))) || Boolean( (mixed_var % 1) );
    };

    // Return number of integers after the decimal point.
    reduxsa.field_objects.slider.decimalCount = function( res ) {
        var q = res.toString().split( '.' );
        return q[1].length;
    };

    reduxsa.field_objects.slider.loadSelect = function( myClass, min, max, res, step ) {
        ...
        for ( var i = min; i <= max; i = i + res ) {
            ...
            var n = i;
            if ( reduxsa.field_objects.slider.isFloat( res ) ) {
                var decCount = reduxsa.field_objects.slider.decimalCount( res );
