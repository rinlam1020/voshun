...
// Don't duplicate me!
    if ( ! class_exists( 'ReduxSAFramework_section' ) ) {

        /**
         * Main ReduxSAFramework_heading class
         *
         * @since       1.0.0
         */
        class ReduxSAFramework_section {
            ...
            public function render() {
                ...
                echo '<div id="section-' . esc_attr($this->field['id']) . '" class="reduxsa-section-field reduxsa-field ' . esc_attr($this->field['style']) . ' ' . esc_attr($this->field['class']) . ' ">';
                ...
                if ( ! empty( $this->field['subtitle'] ) ) {
                    echo '<div class="reduxsa-section-desc">' . esc_html($this->field['subtitle']) . '</div>';
                }
                ...
                <script type="text/javascript">
                    jQuery( document ).ready(
                        function() {
                            ...
                            var group = jQuery( '#<?php echo esc_js($this->field['id']); ?>-marker' ).parents( '.reduxsa-group-tab:first' );
                            if ( !group.hasClass( 'sectionsChecked' ) ) {
                                group.addClass( 'sectionsChecked' );
                                var test = group.find( '.reduxsa-section-indent-start h3' );
                                jQuery.each(
            ...
            public function enqueue() {
                if ( $this->parent->args['dev_mode'] ) {
                    wp_enqueue_style(
                        'reduxsa-field-section-css',
                        ReduxSAFramework::$_url . 'inc/fields/section/field_section.css',
                        array(),
                        time(),
                        'all'
                    );
