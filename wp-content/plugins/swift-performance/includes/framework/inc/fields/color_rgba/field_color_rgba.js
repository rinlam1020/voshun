(function($){
    ...
    reduxsa.field_objects.color_rgba          = reduxsa.field_objects.color_rgba || {};
    reduxsa.field_objects.color_rgba.fieldID  = '';

    reduxsa.field_objects.color_rgba.hexToRGBA = function( hex, alpha ) {
        var result;
    ...
    reduxsa.field_objects.color_rgba.init = function( selector ) {
        if ( !selector ) {
            selector = $( document ).find( ".reduxsa-group-tab:visible" ).find( '.reduxsa-container-color_rgba:visible' );
        }
        ...
        $( selector ).each(
            function() {
                ...
                reduxsa.field_objects.color_rgba.modInit(el);
                reduxsa.field_objects.color_rgba.initColorPicker(el);
            });
    };

    reduxsa.field_objects.color_rgba.modInit = function(el) {

        reduxsa.field_objects.color_rgba.fieldID    = el.find('.reduxsa-color_rgba-container').data('id');

    };

    // Initialize colour picker
    reduxsa.field_objects.color_rgba.initColorPicker = function(el){

        // Get field ID
        var field_id    = reduxsa.field_objects.color_rgba.fieldID;

        // Get the color scheme container
        var colorpickerInput = el.find('.reduxsa-color-rgba');
        ...
        var container   = el.find('.reduxsa-color-rgba-container');
        ...
        colorpickerInput.spectrum({
            ...
            localStorageKey:        'reduxsa.color-rgba.' + field_id,
            palette:                palette,
            inputText:              inputText,
            ...
            change: function(color) {
                ...
                if (colorVal != 'transparent') {
                    rgbaVal     = reduxsa.field_objects.color_rgba.hexToRGBA(colorVal, alphaVal);
                } else {
                    rgbaVal     = 'transparent';
                }
                ...
                reduxsa_change(el.find('.reduxsa-color-rgba-container'));
            }
        });
    };
})(jQuery);