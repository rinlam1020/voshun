<?php
    ...
    // Don't duplicate me!
    if ( ! class_exists( 'ReduxSAFramework_color_rgba' ) ) {

        /**
         * Main ReduxSAFramework_color_rgba class
         *
         * @since       1.0.0
         */
        class ReduxSAFramework_color_rgba {
            ...
            public function __construct( $field = array(), $value = '', $parent ) {
                ...
                $option_defaults = array(
                    ...
                    "choose_text"            => __( 'Choose', 'reduxsa-framework' ),
                    "cancel_text"            => __( 'Cancel', 'reduxsa-framework' ),
                    "show_buttons"           => true,
                    "input_text"             => __( 'Select Color', 'reduxsa-framework' ),
                    "palette"                => null,
                );
            ...
            public function render() {
                ...
                echo '<div 
                      class="reduxsa-color-rgba-container ' . $this->field['class'] . '" 
                      data-id="' . $field_id . '"
                ...
                if ( '' == $this->value['color'] || 'transparent' == $this->value['color'] ) {
                    ...
                    $color = ReduxSA_Helpers::hex2rgba( $this->value['color'], $this->value['alpha'] );
                }

                if ( $this->value['rgba'] == '' && $this->value['color'] != '' ) {
                    $this->value['rgba'] = ReduxSA_Helpers::hex2rgba( $this->value['color'], $this->value['alpha'] );
                }

                echo '<input
                        name="' . $this->field['name'] . $this->field['name_suffix'] . '[color]"
                        id="' . $field_id . '-color"
                        class="reduxsa-color-rgba"
                        type="text"
            ...
            public function enqueue() {
                ...
                // Field dependent JS
                if ( ! wp_script_is( 'reduxsa-field-color-rgba-js' ) ) {
                    wp_enqueue_script(
                        'reduxsa-field-color-rgba-js',
                        ReduxSAFramework::$_url . 'inc/fields/color_rgba/field_color_rgba' . ReduxSA_Functions::isMin() . '.js',
                        array( 'jquery', 'reduxsa-spectrum-js' ),
                        time(),
                        true
                    );
                }
                ...
                // Spectrum CSS
                if ( ! wp_style_is( 'reduxsa-spectrum-css' ) ) {
                    wp_enqueue_style( 'reduxsa-spectrum-css' );
                }

                if ( $this->parent->args['dev_mode'] ) {
                    if ( ! wp_style_is( 'reduxsa-field-color-rgba-css' ) ) {
                        wp_enqueue_style(
                            'reduxsa-field-color-rgba-css',
                            ReduxSAFramework::$_url . 'inc/fields/color_rgba/field_color_rgba.css',
                            array(),
                            time(),
                            'all'
                        );
                ...
                if ( is_array( $this->value ) ) {
                    ...
                    foreach ( $this->value as $id => $val ) {
                        ...
                        if ( $id == 'alpha' ) {
                            ...
                            $rgba = ! empty( $val ) ? $val : '';
                            $rgba = ReduxSA_Helpers::hex2rgba( $color, $alpha );
                        }
                    }
            ...
            public function output() {
                if ( ! empty( $this->value ) ) {
                    ...
                    if ( ! empty( $this->field['output'] ) && is_array( $this->field['output'] ) ) {
                        if ( ! empty( $color_val ) ) {
                            $css = ReduxSA_Functions::parseCSS( $this->field['output'], $style, $color_val );
                    ...
                    if ( ! empty( $this->field['compiler'] ) && is_array( $this->field['compiler'] ) ) {
                        if ( ! empty( $color_val ) ) {
                            $css = ReduxSA_Functions::parseCSS( $this->field['compiler'], $style, $color_val );
