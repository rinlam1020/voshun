...
// Don't duplicate me!
if ( ! class_exists( 'ReduxSAFramework_image_select' ) ) {

    /**
     * Main ReduxSAFramework_image_select class
     *
     * @since       1.0.0
     */
    class ReduxSAFramework_image_select {
        ...
        public function render() {
            ...
            if ( ! empty( $this->field['options'] ) ) {
                echo '<div class="reduxsa-table-container">';
                echo '<ul class="reduxsa-image-select">';
                ...
                foreach ( $this->field['options'] as $k => $v ) {
                    ...
                    $selected = ( checked( $this->value, $theValue, false ) != '' ) ? ' reduxsa-image-select-selected' : '';
                    ...
                    if ( isset( $this->field['presets'] ) && $this->field['presets'] !== false ) {
                        ...
                        $v['presets']['reduxsa-backup'] = 1;

                        $presets   = ' data-presets="' . htmlspecialchars( json_encode( $v['presets'] ), ENT_QUOTES, 'UTF-8' ) . '"';
                        $is_preset = true;

                        $this->field['class'] = trim( $this->field['class'] ) . ' reduxsa-presets';
                    }
                    ...
                    echo '<li class="reduxsa-image-select">';
                    echo '<label class="' . $selected . ' reduxsa-image-select' . $is_preset_class . $this->field['id'] . '_' . $x . '" for="' . $this->field['id'] . '_' . ( array_search( $k, array_keys( $this->field['options'] ) ) + 1 ) . '">';
        ...
        public function enqueue() {
            ...
            wp_enqueue_script(
                'reduxsa-field-image-select-js',
                ReduxSAFramework::$_url . 'inc/fields/image_select/field_image_select' . ReduxSA_Functions::isMin() . '.js',
            ...
            if ($this->parent->args['dev_mode']) {
                wp_enqueue_style(
                    'reduxsa-field-image-select-css',
                    ReduxSAFramework::$_url . 'inc/fields/image_select/field_image_select.css',
                    array(),
                    time(),
                    'all'
                );
