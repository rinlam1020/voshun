...
// Don't duplicate me!
    if ( ! class_exists( 'ReduxSA_Helpers' ) ) {

        /**
         * ReduxSA Helpers Class
         * Class of useful functions that can/should be shared among all ReduxSA files.
         *
         * @since       1.0.0
         */
        class ReduxSA_Helpers {
            ...
            public static function tabFromField( $parent, $field ) {
                foreach ( $parent->sections as $k => $section ) {
                    ...
                    if ( isset( $section['fields'] ) && ! empty( $section['fields'] ) ) {
                        if ( ReduxSA_Helpers::recursive_array_search( $field, $section['fields'] ) ) {
                            return $k;
                            continue;
                        }
                    }
...
//                }
            ...
            public static function trackingObject() {
                ...
                return ReduxSA_Helpers::getTrackingObject();
            }
            ...
            private static function reduxsaAsPlugin() {
                return ReduxSAFramework::$_as_plugin;
            }
            ...
            public static function localize( $localize ) {
                $reduxsa            = ReduxSAFrameworkInstances::get_instance( $localize['args']['opt_name'] );
                $nonce            = wp_create_nonce( 'reduxsa-ads-nonce' );
                $base             = admin_url( 'admin-ajax.php' ) . '?action=reduxsa_p&nonce=' . $nonce . '&url=';
                $localize['rAds'] = ReduxSA_Helpers::rURL_fix( $base, $reduxsa->args['opt_name'] );
            ...
            public static function compileSystemStatus( $json_output = false, $remote_checks = false ) {
                ...
                $sysinfo['reduxsa_ver']      = esc_html( ReduxSAFramework::$_version );
                $sysinfo['reduxsa_data_dir'] = ReduxSAFramework::$_upload_dir;
                $f                         = 'fo' . 'pen';
                // Only is a file-write check
                $sysinfo['reduxsa_data_writeable'] = self::makeBoolStr( @$f( ReduxSAFramework::$_upload_dir . 'test-log.log', 'a' ) );
                ...
                if ( ! class_exists( 'Browser' ) ) {
                    require_once ReduxSAFramework::$_dir . 'inc/browser.php';
                }
                ...
                if ( $remote_checks == true ) {
                    $response = wp_remote_post( 'https://www.paypal.com/cgi-bin/webscr', array(
                        'sslverify'  => false,
                        'timeout'    => 60,
                        'user-agent' => 'ReduxSAFramework/' . ReduxSAFramework::$_version,
                        'body'       => array(
                    ...
                    $response = @wp_remote_get( 'http://reduxsaframework.com/wp-admin/admin-ajax.php?action=get_reduxsa_extensions' );
                ...
                $sysinfo['reduxsa_instances'] = array();

                if ( ! empty( $reduxsa ) && is_array( $reduxsa ) ) {
                    foreach ( $reduxsa as $inst => $data ) {
                        ReduxSA::init( $inst );

                        $sysinfo['reduxsa_instances'][ $inst ]['args']     = $data->args;
                        $sysinfo['reduxsa_instances'][ $inst ]['sections'] = $data->sections;
                        foreach ( $sysinfo['reduxsa_instances'][ $inst ]['sections'] as $sKey => $section ) {
                            if ( isset( $section['fields'] ) && is_array( $section['fields'] ) ) {
                                foreach ( $section['fields'] as $fKey => $field ) {
                                    if ( isset( $field['validate_callback'] ) ) {
                                        unset( $sysinfo['reduxsa_instances'][ $inst ]['sections'][ $sKey ]['fields'][ $fKey ]['validate_callback'] );
                                    }
                                    if ( $field['type'] == "js_button" ) {
                                        if ( isset( $field['script'] ) && isset( $field['script']['ver'] ) ) {
                                            unset( $sysinfo['reduxsa_instances'][ $inst ]['sections'][ $sKey ]['fields'][ $fKey ]['script']['ver'] );
                                        }
                                    }
                        ...
                        $sysinfo['reduxsa_instances'][ $inst ]['extensions'] = ReduxSA::getExtensions( $inst );

                        if ( isset( $data->extensions['metaboxes'] ) ) {
                            $data->extensions['metaboxes']->init();
                            $sysinfo['reduxsa_instances'][ $inst ]['metaboxes'] = $data->extensions['metaboxes']->boxes;
                        }

                        if ( isset( $data->args['templates_path'] ) && $data->args['templates_path'] != '' ) {
                            $sysinfo['reduxsa_instances'][ $inst ]['templates'] = self::getReduxSATemplates( $data->args['templates_path'] );
                        }
                    }
                }
            ...
            private static function getReduxSATemplates( $custom_template_path ) {
                $filesystem         = ReduxSA_Filesystem::get_instance();
                $template_paths     = array( 'ReduxSAFramework' => ReduxSAFramework::$_dir . 'templates/panel' );
                $scanned_files      = array();
                $found_files        = array();
                ...
                foreach ( $scanned_files as $plugin_name => $files ) {
                    foreach ( $files as $file ) {
                        ...
                        if ( $theme_file ) {
                            $core_version  = self::get_template_version( ReduxSAFramework::$_dir . 'templates/panel/' . $file );
                            ...
                            if ( $core_version && ( empty( $theme_version ) || version_compare( $theme_version, $core_version, '<' ) ) ) {
                                ...
                                $found_files[ $plugin_name ][] = sprintf( __( '<code>%s</code> version <strong style="color:red">%s</strong> is out of date. The core version is %s', 'reduxsa-framework' ), str_replace( WP_CONTENT_DIR . '/themes/', '', $theme_file ), $theme_version ? $theme_version : '-', $core_version );
                            } else {
            ...
            public static function rURL_fix( $base, $opt_name ) {
                $url = $base . urlencode( 'http://ads.reduxsaframework.com/api/index.php?js&g&1&v=2' ) . '&proxy=' . urlencode( $base ) . '';

                return ReduxSA_Functions::tru( $url, $opt_name );
            }
                    ...
                    foreach ( $files as $key => $value ) {
                        if ( ! in_array( $value, array( ".", ".." ) ) ) {
                            if ( is_dir( $template_path . DIRECTORY_SEPARATOR . $value ) ) {
                                $sub_files = reduxsa_scan_template_files( $template_path . DIRECTORY_SEPARATOR . $value );
            ...
            public static function get_extension_url( $dir ) {
                $ext_dir = ReduxSA_Helpers::get_extension_dir( $dir );
