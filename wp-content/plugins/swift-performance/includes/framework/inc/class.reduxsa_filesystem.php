<?php
    ...
    if ( ! class_exists( 'ReduxSA_Filesystem' ) ) {
        class ReduxSA_Filesystem {
            ...
            public function __construct() {
                $this->parent->admin_notices[] = array(
                    'type'    => 'error',
                    'msg'     => '<strong>' . __( 'File Permission Issues', 'reduxsa-framework' ) . '</strong><br/>' . sprintf( __( 'We were unable to modify required files. Please check your permissions, or modify your wp-config.php file to contain your FTP login credentials as <a href="%s" target="_blank">outlined here</a>.', 'reduxsa-framework' ), 'https://codex.wordpress.org/Editing_wp-config.php#WordPress_Upgrade_Constants' ),
                    'id'      => 'reduxsa-wp-login',
                    'dismiss' => false,
                );
            }
            ...
            public function ftp_form() {
                if ( isset( $this->parent->ftp_form ) && ! empty( $this->parent->ftp_form ) ) {
                    ...
                    echo '<strong>' . __( 'File Permission Issues', 'reduxsa-framework' ) . '</strong><br/>' . sprintf( __( 'We were unable to modify required files. Please ensure that <code>%1s</code> has the proper read-write permissions, or modify your wp-config.php file to contain your FTP login credentials as <a href="%2s" target="_blank">outlined here</a>.', 'reduxsa-framework' ), ReduxSA_Helpers::cleanFilePath( trailingslashit( WP_CONTENT_DIR ) ) . '/uploads/', 'https://codex.wordpress.org/Editing_wp-config.php#WordPress_Upgrade_Constants' );
            ...
            public function execute( $action, $file = '', $params = '' ) {
                ...
                if ( ! is_dir( ReduxSAFramework::$_upload_dir ) ) {
                    wp_mkdir_p( ReduxSAFramework::$_upload_dir );
                }
		...
		require_once ABSPATH . '/wp-includes/pluggable.php';
                ...
                $url = wp_nonce_url( $base, 'reduxsa-options' );
			...
			if (empty($res) || $res == false || $res == '' ) {
                    ...
                    //add_action( "reduxsa/page/{$this->parent->args['opt_name']}/form/before", array(
                    //    $this,
                    //    'ftp_form'
                    //) );
                }
        ...
        ReduxSA_Filesystem::get_instance();
    }
