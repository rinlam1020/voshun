<?php
    ...
    /**
     * ReduxSA Framework API Class
     * Makes instantiating a ReduxSA object an absolute piece of cake.
     *
    ...
    if ( ! class_exists( 'ReduxSA' ) ) {
        ...
        /**
         * ReduxSA API Class
         * Simple API for ReduxSA Framework
         *
         * @since       1.0.0
         */
        class ReduxSA {
            ...
            public static function load() {
                add_action( 'after_setup_theme', array( 'ReduxSA', 'createReduxSA' ) );
                add_action( 'init', array( 'ReduxSA', 'createReduxSA' ) );
                add_action( 'switch_theme', array( 'ReduxSA', 'createReduxSA' ) );
            }

            public static function init( $opt_name = "" ) {
                if ( ! empty( $opt_name ) ) {
                    self::loadReduxSA( $opt_name );
                    remove_action( 'setup_theme', array( 'ReduxSA', 'createReduxSA' ) );
                }
            }

            public static function loadExtensions( $ReduxSAFramework ) {
                if ( $instanceExtensions = self::getExtensions( $ReduxSAFramework->args['opt_name'], "" ) ) {
                    foreach ( $instanceExtensions as $name => $extension ) {
                        if ( ! class_exists( $extension['class'] ) ) {
                            // In case you wanted override your override, hah.
                            $extension['path'] = apply_filters( 'reduxsa/extension/' . $ReduxSAFramework->args['opt_name'] . '/' . $name, $extension['path'] );
                            if ( file_exists( $extension['path'] ) ) {
                                require_once $extension['path'];
                            }
                        }
                        if ( ! isset( $ReduxSAFramework->extensions[ $name ] ) ) {
                            if ( class_exists( $extension['class'] ) ) {
                                $ReduxSAFramework->extensions[ $name ] = new $extension['class']( $ReduxSAFramework );
                            } else {
            ...
            public static function extensionPath( $extension, $folder = true ) {
                if ( ! isset( ReduxSA::$extensions[ $extension ] ) ) {
                    return;
                }
                $path = end( ReduxSA::$extensions[ $extension ] );
                if ( ! $folder ) {
                    return $path;
                }
            ...
            public static function loadReduxSA( $opt_name = "" ) {

                if ( empty( $opt_name ) ) {
                    return;
                }

                $check = ReduxSAFrameworkInstances::get_instance( $opt_name );
                ...
                if ( ! class_exists( 'ReduxSAFramework' ) ) {
                    echo '<div id="message" class="error"><p>ReduxSA Framework is <strong>not installed</strong>. Please install it.</p></div>';

                    return;
                }
                if ( isset( self::$uses_extensions[ $opt_name ] ) && ! empty( self::$uses_extensions[ $opt_name ] ) ) {
                    add_action( "reduxsa/extensions/{$opt_name}/before", array( 'ReduxSA', 'loadExtensions' ), 0 );
                }

                $reduxsa                   = new ReduxSAFramework( $sections, $args );
                $reduxsa->apiHasRun        = 1;
                self::$init[ $opt_name ] = 1;
                if ( isset( $reduxsa->args['opt_name'] ) && $reduxsa->args['opt_name'] != $opt_name ) {
                    self::$init[ $reduxsa->args['opt_name'] ] = 1;
                }

            }

            public static function createReduxSA() {
            ...
            public static function setSections( $opt_name = '', $sections = array() ) {
                ...
                if ( ! empty( $sections ) ) {
                    foreach ( $sections as $section ) {
                        ReduxSA::setSection( $opt_name, $section );
                    }
                }
            }
            ...
            public static function getOption( $opt_name = "", $key = "" ) {
                ...
                if ( ! empty( $opt_name ) && ! empty( $key ) ) {
                    $reduxsa = get_option( $opt_name );

                    if ( isset( $reduxsa[ $key ] ) ) {
                        return $reduxsa[ $key ];
                    } else {
                        return;
                    }
            ...
            public static function setOption( $opt_name = "", $key = "", $option = "" ) {
                ...
                if ( ! empty( $opt_name ) && ! empty( $key ) ) {
                    ...
                    return update_option( $opt_name, $reduxsa );
                } else {
                    return false;
                }
            }
            ...
            public static function checkExtensionClassFile( $opt_name, $name = "", $class_file = "", $instance = "" ) {
                if ( file_exists( $class_file ) ) {
                    ...
                    $version                   = ReduxSA_Helpers::get_template_version( $class_file );
                    ...
                    if ( file_exists( $api_check ) && ! class_exists( 'ReduxSA_' . ucfirst( $name ) ) ) {
            ...
            public static function getAllExtensions() {
                ...
                foreach ( $reduxsa as $instance ) {
                    if ( ! empty( self::$uses_extensions[ $instance['args']['opt_name'] ] ) ) {
                        continue;
                    }
                    if ( ! empty( $instance['extensions'] ) ) {

                        ReduxSA::getInstanceExtensions( $instance['args']['opt_name'], $instance );
                    }
                }
            }
            ...
            public static function getInstanceExtensions( $opt_name, $instance = array() ) {
                ...
                if ( empty( $instance ) ) {
                    $instance = ReduxSAFrameworkInstances::get_instance( $opt_name );
                }
                ...
                foreach ( $instance->extensions as $name => $extension ) {
                    if ( $name == "widget_areas" ) {
                        $new = new ReduxSA_Widget_Areas( $instance );
                    }
                    if ( isset( self::$uses_extensions[ $opt_name ][ $name ] ) ) {
                        continue;
                    }
                    if ( isset( $extension->extension_dir ) ) {
                        ReduxSA::setExtensions( $opt_name, str_replace( $name, '', $extension->extension_dir ) );

                    } else if ( isset( $extension->_extension_dir ) ) {
                        ReduxSA::setExtensions( $opt_name, str_replace( $name, '', $extension->_extension_dir ) );
                    }
                }
            }
            ...
            public static function getExtensions( $opt_name = "", $key = "" ) {
                ...
                if ( empty( $opt_name ) ) {
                    ...
                    if ( empty( self::$uses_extensions[ $opt_name ] ) ) {
                        ReduxSA::getInstanceExtensions( $opt_name );
                    }
                    ...
                    foreach ( self::$uses_extensions[ $opt_name ] as $extension ) {
                        ...
                        $extension_class                  = 'ReduxSAFramework_Extension_' . $name;
